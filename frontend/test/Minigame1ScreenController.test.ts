...
describe("Minigame1ScreenController (simple)", () => {
  const fakeSwitcher = { switchToScreen: vi.fn() };

  it("startGame() calls model.reset and displays first sequence", () => {
    const controller = new Minigame1ScreenController(fakeSwitcher as any, {} as any);

    const model = (controller as any).model;
    const view = (controller as any).view;

    controller.startGame();

    expect(model.reset).toHaveBeenCalled();
    expect(model.getSequence).toHaveBeenCalled();
    expect(view.displayQuestion).toHaveBeenCalled();
  });
  ...
  it("handleAnswer() calls model.checkAnswer", () => {
    ...
    expect(model.checkAnswer).toHaveBeenCalledWith(10);
  });
}); 
